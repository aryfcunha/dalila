...
if digests_dir.exists():
    ...
    for f in html_files:
        ...
        if date_str not in existing_digests:
            ...
            composed_at = dt.isoformat() + "+00:00"
            date_label = dt.strftime("%A %d %B %Y")
            inserts_digest.append((composed_at, date_label, "", "[]", 0))
    
    if inserts_digest:
        c.executemany("INSERT INTO digests (composed_at, date_label, content, item_ids_json, item_count) VALUES (?, ?, ?, ?, ?)", inserts_digest)
