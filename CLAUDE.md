...
- **Haiku (CLI) is primary; DeepSeek is an automatic fallback for hard CLI failures only.** The live path is Haiku via the `claude` CLI. `llm._run_claude` falls back to DeepSeek (`src/dalila/deepseek.py`) *only* when the CLI genuinely can't run a call — binary missing, auth broken, timeout, malformed output — and *only* if `DEEPSEEK_API_KEY` is set. A routine Haiku **rate-limit does NOT fall back**: `_looks_like_rate_limit()` re-raises it so the scheduler backs off and retries on Haiku at $0 (spilling rate-limits to paid DeepSeek is the cost trap that silently ran up spend). `DALILA_LLM_BACKEND=deepseek` still exists but is a **manual, opt-in force** for one-shot cost-bounded backfills only — never set it on the live VM. `dalila classify --backend deepseek` remains the explicit backfill path. Leaving `DEEPSEEK_API_KEY` blank disables the fallback entirely (Haiku-only).
