...
- **ACAPS INFORM auth fix** (2026-06-26): the INFORM ingestor used a static `ACAPS_API_KEY` as a Bearer token — but ACAPS has no static key; it's **token-exchange**. `inform.py` now POSTs `ACAPS_USERNAME`+`ACAPS_PASSWORD` to `/api/v1/token-auth/` (cached in-process) and sends `Authorization: Token <token>`. Other corrections verified against the live API: the endpoint **rejects** `_format`/`limit` params (404 "Field X is not valid") — paginate via the DRF `next` URL instead; it 302-redirects to the current month; the score field is **`INFORM Severity Index` on a 0–10 scale** (threshold raised to 1.0, units `/10`), and `iso3`/`country` come as **lists** (normalized via `_first` before `resolve_iso2`). Config gained `acaps_username`/`acaps_password` (set in `.env`; `ACAPS_API_KEY` now legacy/ignored). `dalila check` reflects the new creds. Live-verified: 111 crises fetched, 67 country baselines recorded (Sudan/Myanmar 9.5, DRC 9.2…).
...
- **Commitment provenance: "UAE" → "Development" financial commitments** (2026-06-25): broadened `financial_commitments` from UAE-donor-centric to a directed source→beneficiary edge (telemetry for a future commitment-flows page). Migration `009_commitment_provenance.sql` adds 4 nullable columns (`source_country`, `source_entity`, `beneficiary_country`, `beneficiary_entity`) + a `(source_country, beneficiary_country)` flow index. `prompts/classifier.md` "Financial commitments" → "Development financial commitments" — donor no longer must be the UAE; extracts both sides. `FinancialCommitment` (models.py) gains the 4 fields; `recipient` retained and mirrored to/from `beneficiary_entity` for back-compat. `db.save_classification` INSERT + `recent_financial_commitments` SELECT carry the new columns. Bot `/commitments` renders `source → beneficiary · via fund` and is relabelled "Development financial commitments". Fully backward-compatible (all new columns nullable; old rows keep `recipient`).
- **GDELT translingual ingestion (multi-language coverage)** (2026-06-25): GDELT only read the English GKG, so the ~200 foreign-language outlets on `trusted_outlets.yaml` never entered the pipeline. `gdelt.fetch` now also pulls the **translingual** feed (`lastupdate-translation.txt` → `.translation.gkg.csv.zip`, same schema), gated by `translingual` on the source (default on; `translingual: true` set on `gdelt_v2`). Refactor: extracted `_download_gkg_text(lastupdate_url)` + `_rows_to_items(text, source_id, allowlist)` shared by both feeds; `url_hash` dedup collapses overlap. Works with the existing pipeline because GKG themes/orgs/persons are English regardless of article language (so the English prefilter still matches), and the classifier already emits `translated_title` which `save_classification` uses to overwrite the foreign slug title with a clean English headline. (No new column needed — translation display already worked; the gap was purely that no foreign items were being ingested.)
