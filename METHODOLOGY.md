...
| Outlet interleaving | **Every publisher&rsquo;s best item is considered before any publisher&rsquo;s second item** | The candidate list is re-ordered breadth-first across outlets (quality still ranks within each round), so the brief draws on the full breadth of gathered sources rather than the few highest-volume ones. |
